...
export const startersApi = {
  ...
  update: (id: number, data: Partial<{ name: string; description: string; hydration_percent: number }>) =>
  ...
  addFeeding: (
    starterId: number,
    data: { flour_grams?: number; water_grams?: number; starter_grams?: number; notes?: string },
  ) =>
